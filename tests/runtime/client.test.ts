...
function setup() {
  ...
  const orpc = createORPCNuxtClient(rawClient, { queryClient })
  return {
    queryClient,
    scope,
    orpc,
...
describe("query and mutation client", () => {
  ...
  test("invalidation keeps clients with different prefixes separate", async () => {
    ...
    const blog = createORPCNuxtClient(rawClient, { queryClient, prefix: "blog" })
    const archive = createORPCNuxtClient(rawClient, { queryClient, prefix: "archive" })
  ...
  test("captures an injected client for invalidation after the plugin context has ended", async () => {
    ...
    const orpc = app.runWithContext(() => createORPCNuxtClient(rawClient))
