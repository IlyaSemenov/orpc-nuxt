...
const workspace = await mkdtemp(join(tmpdir(), "orpc-vue-"))
...
const requiredPeers = Object.fromEntries(
  Object.entries(pkg.peerDependencies).filter(([name]) => !optionalPeers.has(name)),
)
...
try {
  ...
  const archive = join(workspace, "orpc-vue.tgz")
  await run(
  ...
  for (const { version, owner } of scenarios) {
    ...
    await writeFile(
      ...
      JSON.stringify(
        {
          name: "orpc-vue-consumer",
          private: true,
          type: "module",
          dependencies: {
            "orpc-vue": `file:${archive}`,
            ...requiredPeers,
            nuxt: version,
