...
const post = orpc.blog.posts.get.useQuery(() => ({ id: postId.value }), { enabled })
const failure = orpc.blog.posts.fail.useQuery(undefined, { enabled: false })
const { data: manualUser } = useNuxtApp().$manualOrpc.blog.users.get.useQuery()
// Both observers share one cache entry; editing the clone must leave the selected name intact.
const query = await orpc.hello.useQuery(undefined, { clone: true })
const selected = orpc.hello.useQuery(undefined, { select: (data) => data.name })
// Awaiting an inactive query must not block SSR or component setup.
const clientOnly = await orpc.hello.useQuery(undefined, {
  server: false,
  enabled: false,
})
...
<template>
  <main>
    ...
    <button id="invalidate" @click="orpc.blog.posts.invalidate()">Refresh posts</button>
