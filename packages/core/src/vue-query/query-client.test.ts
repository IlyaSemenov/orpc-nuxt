...
import { hashKey, QueryClient, VUE_QUERY_CLIENT, VueQueryPlugin } from "@tanstack/vue-query"
import { renderToString } from "@vue/server-renderer"
import { createSSRApp, defineComponent, h, provide } from "vue"

import { captureQueryClient, resolveQueryClient } from "./query-client"
...
test("captures an injected client for use after the plugin context has ended", () => {
  const queryClient = new QueryClient()
  const explicit = new QueryClient()
  const app = createSSRApp({ render: () => null })
  app.use(VueQueryPlugin, { queryClient })
  const injected = app.runWithContext(() => captureQueryClient(undefined, undefined, hashKey))
  const preferred = app.runWithContext(() => captureQueryClient(explicit, undefined, hashKey))
  expect(injected()).toBe(queryClient)
  expect(preferred()).toBe(explicit)
})
